...
## Recent Changes (April 11, 2026)

✅ **Refactored Item Editor**
- Moved `ItemEditor.tsx` from `/admin/items/[id]/` to `/src/components/`
- Deleted orphaned `/admin/items/[id]/` route (no incoming links)
- Updated `CreateItemModal` to import from new location

✅ **Improved Image Cropping**
- Default crop size now matches frame size (309px)
- Crop slider limited to frame size, not image size
- Reduced image quality to JPEG 0.8 to prevent serialization errors

✅ **Security Fix**
- DevTools now disabled in production
- Only enabled in development for debugging

✅ **Database Utilities**
- Added `import-cocktails.ts` for batch menu import
- Added `update-password.ts` for password updates
- Deleted "Cucumber Gin" item from inventory

✅ **Documentation**
- Comprehensive README with deployment instructions
- This CLAUDE.md with architecture details
...
- [ ] Git cloned
- [ ] `.env.dev` and `.env.prod` created with valid DB credentials
- [ ] NEXTAUTH_SECRET generated and set
- [ ] `make run` succeeds with "Dev environment is ready!"
- [ ] Database migrations run: `make migrate`
- [ ] (Optional) Load backup: `make load FILE=backups/latest.sql`
